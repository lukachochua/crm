<?php

namespace App\Policies\Hr;

use App\Models\Hr\Survey\SurveySubmission;
use App\Models\User;
use App\Policies\Hr\Concerns\ScopesHrAccess;
use App\Support\Permissions;

class SurveySubmissionPolicy
{
    use ScopesHrAccess;

    public function viewAny(User $user): bool
    {
        return $user->can(Permissions::permission('survey_submissions', 'view'));
    }

    public function view(User $user, SurveySubmission $submission): bool
    {
        if (! $user->can(Permissions::permission('survey_submissions', 'view'))) {
            return false;
        }

        if ($this->canBypassScope($user)) {
            return true;
        }

        if ($this->isHrManager($user) || $this->isDepartmentManager($user)) {
            return $this->scopedEmployeeByUserId($user, $submission->user_id);
        }

        return true;
    }

    public function create(User $user): bool
    {
        return $user->can(Permissions::permission('survey_submissions', 'create'));
    }

    public function update(User $user, SurveySubmission $submission): bool
    {
        if (! $user->can(Permissions::permission('survey_submissions', 'update'))) {
            return false;
        }

        if ($this->canBypassScope($user)) {
            return true;
        }

        if ($this->isHrManager($user) || $this->isDepartmentManager($user)) {
            return $this->scopedEmployeeByUserId($user, $submission->user_id);
        }

        return true;
    }

    public function delete(User $user, SurveySubmission $submission): bool
    {
        if (! $user->can(Permissions::permission('survey_submissions', 'delete'))) {
            return false;
        }

        if ($this->canBypassScope($user)) {
            return true;
        }

        if ($this->isHrManager($user) || $this->isDepartmentManager($user)) {
            return $this->scopedEmployeeByUserId($user, $submission->user_id);
        }

        return true;
    }

    public function export(User $user): bool
    {
        return $user->can(Permissions::permission('survey_submissions', 'export'));
    }

    public function restore(User $user, SurveySubmission $submission): bool
    {
        return false;
    }

    public function forceDelete(User $user, SurveySubmission $submission): bool
    {
        return false;
    }
}
